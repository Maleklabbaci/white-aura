...
// ✅ Auto-inscription + ajout de points (tout en 1)
export async function addPoints(phone: string, points: number = 1, clientName: string = '') {
  try {
    const phoneClean = phone.replace(/\s/g, '')
    const res = await fetch(`${FIDALI_CONFIG.apiUrl}/auto-points`, {
      method: 'POST',
      headers: headers(),
      body: JSON.stringify({
        card_code: FIDALI_CONFIG.cardCode,
        phone: phoneClean,
        name: clientName || 'Client White Aura',
        points,
      }),
    })
...
export async function getClientPoints(phone: string) {
  try {
    ...
    if (data.error || !data.cards || !data.cards.length) return null
    return { name: data.name, phone: data.phone, card: data.cards[0] }
  } catch (e) {
